...
def main() -> None:
	...
	for set_idx, measurement_nodes in enumerate(measurement_sets, start=1):
		...
		if RADIUS:
			...
			solver_mode = SOLVER
			if SOLVER in {"ipopt", "global", "hexaly", "hexaly_xd"}:
				try:
			...
			if solver_mode in {"hexaly", "hexaly_xd"}:
				multi_starts = 1
				multi_noise = 0.0
			else:
				multi_starts = MULTI_STARTS
				multi_noise = MULTI_START_NOISE
			...
			for run_idx in range(max(1, multi_starts)):
				...
				if multi_starts > 1:
					for k in q0:
						q0[k] = q0[k] + multi_noise * rng.standard_normal()
					for k in h0:
						h0[k] = h0[k] + multi_noise * rng.standard_normal()
	...
	if summary_rows and write_summary:
		def _summary_sort_key(row: Dict[str, object]) -> tuple[bool, float]:
			val = row.get("radius")
			if val is None:

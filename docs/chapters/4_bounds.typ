...
$
	Y ~_( S) Z quad :<==> quad &  h_S^Y = h_S^Z "and" d_w^Y = d_w^Z
$
That is, they have the same pressures at the measurement sites $S$ and the same total demand at the reservoir $R$, so measurements at $S$ cannot distinguish between them.
...
The equivalence class $[Y]_~$ is uniquely determined by the values of $h_S^Y$ and $d_w^Y$, thus we may identify it with the _measurement_ $M =(h_S^Y, d_w^Y)$. Hence, by abuse of notation, we write $Y in M$, if $Y$ is a scenario with measurements $M$.

Given a fixed measurement $M$, we define:
$
  &W_d (M) &&:= sup_(Y,Z in M) thick norm(d^Y - d^Z) quad quad 
  quad && ("Worst-case demand distance at" M) \
  &C_d (M) &&:= inf_(Y in M) thin sup_(Z in M) thick norm(d^Y - d^Z) 
  && ("Chebyshev demand distance at" M) \
  &W_d &&:= sup_(M) thick W_d (M) = sup_(Y ~ Z) thick norm(d^Y - d^Z) 
  quad && ("Worst-case demand distance") \
  &C_d &&:= sup_(M) thick C_d (M)
    && ("Chebyshev demand distance")
$
